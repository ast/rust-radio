...
async fn insert_and_retrieve_session() {
    ...
    assert_eq!(store.get_username("token123").await, Some("sm6wjm".to_string()));
}

#[tokio::test]
async fn unknown_token_returns_none() {
    let store = SessionStore::new();
    assert_eq!(store.get_username("nonexistent").await, None);
}
...
async fn remove_session() {
    ...
    assert_eq!(store.get_username("token123").await, None);
}
...
async fn multiple_sessions() {
    ...
    assert_eq!(store.get_username("t1").await, Some("user1".to_string()));
    assert_eq!(store.get_username("t2").await, Some("user2".to_string()));
}
